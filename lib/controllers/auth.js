...
function getGoogleCallbackURL(request) {
  var protocol = firstHeaderValue(request.headers['x-forwarded-proto']) || config.app.url.protocol;
  var host = firstHeaderValue(request.headers['x-forwarded-host']) ||
    firstHeaderValue(request.headers.host);

  if (!host) {
    host = config.app.url.hostname;
    if (config.app.url.port) {
      host += ':' + config.app.url.port;
    }
  }
